...
def conv_block(in_channels, out_channels, kernel_size):
    return nn.Sequential(nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, padding='same'),
                         nn.LeakyReLU(),
...
class UNet(nn.Module):
    def __init__(self):
        ...
        self.conv1 = conv_block(1, 16, 5)
        self.conv2 = conv_block(16, 32, 5)
        self.conv3 = conv_block(32, 64, 5)
        self.conv4 = nn.Conv2d(64, 2, kernel_size=3, padding='same')
        self.pool = nn.MaxPool2d(2)
        self.transpose1 = nn.ConvTranspose2d(32, 32, kernel_size=3, stride=2, padding=0)
        self.transpose2 = nn.ConvTranspose2d(80, 64, kernel_size=3, stride=2, padding=1)
    ...
    def forward(self, x):
        ...
        #x = self.bn1(x)
        #print(x.shape)
        x1 = torch.nn.functional.pad(x, (1, 0, 0, 1))
        #print(x1.shape)
        #x = self.relu(x)
        x = self.conv2(x)
        x = self.pool(x)
        ...
        #x = self.relu(x)
        x = self.transpose2(x)[:,:,:-1,:-1]
        #print(x.shape)
        #x = self.relu(x)
        x = self.conv4(x)
        return x
net = UNet()
net(torch.randn([4, 1, 100, 100])).shape
...
class UNet16(nn.Module):
    def __init__(
        ...
        self.encoder[0] = nn.Conv2d(1, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
